...
def send_welcome(message):
    if database.is_user_new(user_id=message.chat.id):
        ...
        bot.send_message(chat_id=message.chat.id, text=Welcome_message)  # welcome message
...
def echo_all(message):
    if database.get_state(user_id=message.chat.id) != 'waiting':
        ...
        if len(urls) == 0:
            bot.reply_to(message, answer_for_message_wo_links)  # answer for messages w/o links
        #     TODO all texts to content file
        else:
            database.set_state(user_id=message.chat.id, state='waiting')
            bot.reply_to(message, answer_for_message_with_links)  # answer for links it gets
            for url in urls:
